...
const int pinoDHT11 = 12; //PINO ANALÓGICO UTILIZADO PELO DHT11
...
/* - Device Address */
static const u4_t DEVADDR = ; //Colocar o Device Address

/* - Tempo entre envios de pacotes LoRa */
const unsigned TX_INTERVAL = 600; //1800s = 30 minutos 
...
void writeToDisplay(char * str)
{
  ...
  display.drawString(0 , 0 , "@JorgeMaia");
...
{
    writeToDisplay("Enviando...");
    static uint8_t mydata[60];
    char mydata_str[60]={0};
    int temp = (int)(random(18,35));
    int hum = (int)(random(70,75));
    int lux = (int)random(700,950);

    /* Formata dado a ser enviado (número aleatório de 0 a 99) */
    // {temp: 27, hum: 40}
    // DHT.read11(pinoDHT11);
    sprintf(mydata_str, "{\"Local\":\"Primeiro Andar\",\"T\":%d,\"U\":%d,\"L\":%d}", temp, hum, lux);  
...
void setup() 
{
    ...
    display.clear();
    display.setTextAlignment(TEXT_ALIGN_LEFT);
    display.drawString(0 , 0 , "CrazyTechLabs");
    display.display();
