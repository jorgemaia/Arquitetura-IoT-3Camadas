...
static void pppos_client_task()
{
	...
	uart_param_config(uart_num, &uart_config);// goto exit;
	//Set UART1 pins(TX, RX, RTS, CTS)
	uart_set_pin(uart_num, UART_GPIO_TX, UART_GPIO_RX, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);// goto exit;
	uart_driver_install(uart_num, BUF_SIZE * 2, BUF_SIZE * 2, 0, NULL, 0);// goto exit;
	//==============================
	End of uart port setting*/
...
int enableGPS()
{
	...
	ESP_LOGI("GPS", "ENABLED.");
	atCmd_waitResponse("AT+CGNSPWR=1\r\n", GSM_OK_Str, NULL, 14, 10000, NULL, 0);

	int res = atCmd_waitResponse("AT+CGPSSTATUS?\r\n", NULL, NULL, 16, 2000, &pbuf, 63);
	ESP_LOGI("GPS", "STATUS ASWR -> %s.", pbuf);
	
	if (res > 0) {
		if (strstr(buf, "Location 3D Fix") != NULL){
			return 1;
		}
	}
	return 0;
...
int disableGPS()
{
	ESP_LOGI("GPS", "DISABLED.");
	return atCmd_waitResponse("AT+CGNSPWR=0\r\n", GSM_OK_Str, NULL, 14, 10000, NULL, 0);
}

int getGPS(double* lat, double* lon)
{
	ESP_LOGI("GPS", "GETTING GPS SIGNAL.");

	char buf[128] = {'\0'};
	char *rbuffer = buf;
	
	ESP_LOGI("GPS", "SET RECEP. BUFFER.");

	//int res = atCmd_waitResponse("AT+CGPSINF=4\r\n", NULL, NULL, 14, 2000, &rbuffer, 127);
	int res = atCmd_waitResponse("AT+CGNSINF\r\n", NULL, NULL, 12, 2000, &rbuffer, 127);
	
	ESP_LOGI("GPS", "RES = %d.", res);
	if (res <= 0) {
		free(rbuffer);
		return 0;
	}

	ESP_LOGI("GPS", "GOT COORDINATES.");
	ESP_LOGI("GPS", "RAW DATA = %s.", rbuffer);

	int ngps = strlen((const char*) rbuffer);

	if(ngps > 0){

		if(strstr(buf, "ERROR") == NULL){

			//nova forma de pegar lat long
			//ignora os 3 primeiros valores
			//strtok pega o próximo valor separado pelo token da ultima string passada quando recebe NULL
			char* token = ",";
			strtok(rbuffer, token);
			strtok(NULL, token); 
			strtok(NULL, token);
			//o quarto valor é a latitude
			char* latS = strtok(NULL, token);
			//o quinto valor é a longitude
			char* lonS = strtok(NULL, token);

			double latD, lonD;
			sscanf(latS, "%lf", &latD);
			sscanf(lonS, "%lf", &lonD);
			
			if((latD == 0.0) || (lonD == 0.0) || (lat == NULL) || (lon == NULL)) return 0;
			*lat = latD;
			*lon = lonD;


		}else{
			ESP_LOGI("GPS", "ERROR WITH COORDINATES.");
			return 0;
		}

		ESP_LOGI("GPS", "LAT-> %lf.", *lat);
		ESP_LOGI("GPS", "LON-> %lf.", *lon);
		counter = 0;
		return 1;
	}else{
		ESP_LOGI("GPS", "NO BYTES RECEIVED.");
		return 0;
	}
}
...
int get_timeGPS()
{
	...
	atCmd_waitResponse("AT+CREG=1\r\n", GSM_OK_Str, NULL, 11, 10000, NULL, 0);

	atCmd_waitResponse("AT+CLTS=1\r\n", GSM_OK_Str, NULL, 11, 10000, NULL, 0);

	int res = atCmd_waitResponse("AT+CCLK?\r\n", NULL, NULL, 10, 2000, &pbuf, 127);

	ESP_LOGI("GPS", "RAW TIME DATA = %s.", pbuf);
	...
	char stamp[128];
	sprintf_s(stamp,sizeof(stamp), "%s-%s-%s %s:%.0lf:%.0lf",year,mon,day,hour,mi,s);
	ESP_LOGI("GPS", "Timestamp -> %s\n", stamp);

	struct tm tm = { 0 };
	time_t t = 0;
	if ( strptime(stamp, "%Y-%m-%d %H:%M:%S", &tm) != NULL ){
  		t = mktime(&tm);
		...
		time(&t);
		localtime_r(&t,&tm);
		ESP_LOGI("GPS", "TIME: NEW TIME SET TO %s", asctime(&tm));
	}else{
  		// badness
		ESP_LOGI("GPS", "TIME: Problem updating timestamp.");
		return 0;
	}
